    ...
    if (group === 0) {
      ...
      if (selectYear !== year) {
        ...
        if (months.length !== newMonths.length) {
          var minMonth = Number.parseInt(newMonths[0], 10);
          var maxMonth = Number.parseInt(newMonths[newMonths.length - 1], 10);
          state.selectMonth = (0, _utilsUtil.valueBetween)({
            max: maxMonth,
            min: minMonth,
            value: selectMonth
          });
        ...
        if (days.length !== newDays.length) {
          var minDay = Number.parseInt(newDays[0], 10);
          var maxDay = Number.parseInt(newDays[newDays.length - 1], 10);
          state.selectDay = (0, _utilsUtil.valueBetween)({
            max: maxDay,
            min: minDay,
            value: selectDay
          });
      ...
      var startD = startYear === selectYear && month === startMonth ? startDay : 1;
      var endD = endYear === selectYear && month === endMonth ? endDay : 31;
      var newDays = this._getDaysInMonth(month, selectYear, startD, endD);
      this.setState({
        days: newDays,
        selectMonth: month,
